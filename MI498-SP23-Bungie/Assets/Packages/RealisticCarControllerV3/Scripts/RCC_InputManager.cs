...
public class RCC_InputManager {
    ...
    public static RCC_Inputs GetInputs() {
        ...
        if (!RCC_Settings.Instance.mobileControllerEnabled && !CarManager.Instance.autoDriving) {
            bool reverse = Input.GetButton("Fire1") || Input.GetKey(KeyCode.R);
            int reverse_int = reverse ? -1 : 1;
            
            inputs.throttleInput = inputActions.Vehicle.Throttle.ReadValue<float>();  // 1 for holding W, 0 otherwise
            inputs.brakeInput = inputActions.Vehicle.Brake.ReadValue<float>(); // 1 for holding S, 0 otherwise
            inputs.steerInput = inputActions.Vehicle.Steering.ReadValue<float>();  // A is -1, D is 1
            if (CarManager.currentState == CarManager.CarState.TiltingLeft || CarManager.currentState == CarManager.CarState.TiltingRight)
            {
                inputs.steerInput *= 0.5f;
            }
            if (CarManager.Instance.vrMode)
            {
                var test = SteeringWheel.wheelTransform.rotation.z;
                inputs.steerInput = SteeringWheel.wheelTransform.rotation.z / (-Mathf.PI / 2);
            }
            // COMMENT THIS OUT TO GO BACK TO REGULAR CONTROLS
            /*
            // Camera based input here (braking handled by handbrake or SPACE since S now goes "down")
            //float camThrottleInput = reverse ? 0 : Mathf.Min(inputs.throttleInput + inputs.brakeInput + Mathf.Abs(inputs.steerInput), 1);
            //Vector2 heading = new Vector2(CarManager.heading.z, -CarManager.heading.x);
            //Vector2 desiredDirection = new Vector2(inputs.steerInput, inputs.throttleInput - inputs.brakeInput).normalized;

            //inputs.throttleInput = camThrottleInput;
            //inputs.brakeInput = reverse ? 1 : 0;
            //inputs.steerInput = Vector2.Dot(heading, desiredDirection) * reverse_int;
            */
            // END COMMENT OUT
