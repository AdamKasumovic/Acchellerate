...
public class EnemyStats : MonoBehaviour
{
    ...
    public int Damage(float damageTaken)
    {
        ...
        if (health <= 0)
        {
            ...
            gameObject.transform.GetChild(0).GetComponent<ZombieRagdollHandler>().EnableRagdoll(true,true, (!NukeManager.Instance.nukeActive && CarManager.Instance.shouldGib || (CarManager.Instance.boost && !CarManager.Instance.boostRefreshing)) && !CarManager.Instance.groundPoundKillIndicator);
            return pointReward;
        }
        return 0;
    }
